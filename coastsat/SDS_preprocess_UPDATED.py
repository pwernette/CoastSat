...
def get_reference_sl(metadata, settings):
    ...
    # otherwise get the user to manually digitise a shoreline on S2, L8 or L5 images (no L7 because of scan line error)
        ...
        elif not 'S2' in metadata.keys() and 'HiRes' in metadata.keys():
            satname = 'HiRes'
            filepath = SDS_tools.get_filepath(settings['inputs'],satname)
            filenames = metadata[satname]['filenames']
        elif not 'S2' in metadata.keys() and not 'HiRes' in metadata.keys() and 'L8' in metadata.keys():
            satname = 'L8'
            filepath = SDS_tools.get_filepath(settings['inputs'],satname)
            filenames = metadata[satname]['filenames']
        ...
        elif not 'S2' in metadata.keys() and not 'HiRes' in metadata.keys() and not 'L8' in metadata.keys() and 'L5' in metadata.keys():
            satname = 'L5'
            filepath = SDS_tools.get_filepath(settings['inputs'],satname)
            filenames = metadata[satname]['filenames']
        else:
